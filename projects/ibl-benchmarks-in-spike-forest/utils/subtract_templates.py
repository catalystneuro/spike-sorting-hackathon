...
class SubtractTemplatesRecording(BasePreprocessor):
    ...
    def __init__(self, recording, sorting, templates_dict, n_before, good_units=None,
        ...
        self._kwargs = dict(recording=recording.to_dict(), sorting=sorting.to_dict(), 
...
class SubtractRecordingSegment(BasePreprocessorSegment):
    ...
    def __init__(self, segment, all_spikes, all_labels, templates_dict, n_before, unit_ids, verbose):
    ...
    def get_traces(self, start_frame, end_frame, channel_indices):
        ...
        # TODO: improve search
        # TODO: add margins
        # TODO: improve efficiency (precompute)
        ...
        spike_windows_loop = range(len(spikes_window))
        
        if self.verbose:
            spike_windows_loop = tqdm(spike_windows_loop, ascii=True, desc="Subtracting spikes")
        
        for i in spike_windows_loop:
            if labels_window[i] in self.unit_ids:
                st = spikes_window[i]
                temp = np.array(self.templates[labels_window[i]]).astype(dtype)
...
subtract_templates = define_function_from_class(source_class=SubtractTemplatesRecording, name="subtract_templates")
